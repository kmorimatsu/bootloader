...
void __ISR(_TIMER_2_VECTOR,IPL7SOFT) T2Handler(void){
	...
	if (42*2+9*2<=synctable_point && synctable_point<=241*2+9*2) {
		...
		// Available x values are 0x06, 0x08, 0x0a, 0x0c, 0x0e, 0x10, 0x12, 0x14,
		// 0x16, 0x18, 0x1a, 0x1c, 0x1e, 0x00, 0x02, and 0x04.
		if (0x06==x) {
			// Read A/D converter values connected to keyboard.
			...
			// Construct different type of key matrix for easy reading key values
...
void printstr(int cursor,char* str){
	int i;
	unsigned char c;
	for(i=0;c=str[i];i++) VRAM[cursor+i]=c;
}
void printhex4(int cursor, unsigned char val){
	if (val<10) VRAM[cursor]=val+0x30;
	else VRAM[cursor]=val+(0x41-10);
}
